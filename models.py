...
class DeviceInstanceRange(BaseModel):
    start_instance: conint(ge=0, le=4194303) = Field(
        ..., description="The start of the BACnet device instance range."
    )
    end_instance: conint(ge=0, le=4194303) = Field(
        ..., description="The end of the BACnet device instance range."
    )
...
class WritePropertyRequest(BaseModel):
    ...
    def validate_property_identifier(cls, v):
        ...
        if v not in valid_property_identifiers:
            raise ValueError(
                f"property_identifier '{v}' is not a valid BACnet property identifier"
            )
        return v
    ...
    def validate_object_identifier(cls, v):
        if "," not in v:
            raise ValueError(
                "object_identifier must include a type and an instance number separated by a comma"
            )
        ...
        if object_type not in valid_object_type_identifiers:
            raise ValueError(
                f"object_identifier '{object_type}' is not a valid BACnet object type"
            )
...
class DeviceInstanceValidator(BaseModel):
    device_instance: conint(ge=0, le=4194303) = Field(
        ..., description="The device instance ID like '201201' for example"
    )
