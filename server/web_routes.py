...
def setup_routes(app: FastAPI, bacnet_app):
    ...
    async def read_index(request: Request):
        user_logged_in = "user" in request.session and request.session["user"] in bacnet_app.users
        return bacnet_app.templates.TemplateResponse("index.html", {"request": request, "user_logged_in": user_logged_in})
    ...
    async def login(request: Request, form_data: OAuth2PasswordRequestForm = Depends()):
        user_dict = bacnet_app.users.get(form_data.username)
        if not user_dict or user_dict["password"] != form_data.password:
            return bacnet_app.templates.TemplateResponse("login.html", {"request": request, "error": "Invalid credentials"})
        request.session["user"] = form_data.username
        return RedirectResponse(url="/", status_code=status.HTTP_302_FOUND)

    @app.get("/dashboard", response_class=HTMLResponse)
    async def read_dashboard(request: Request, user: dict = Depends(get_current_active_user)):
        return bacnet_app.templates.TemplateResponse("dashboard.html", {"request": request, "user": user})

    @app.get("/schedule", response_class=HTMLResponse)
    async def read_schedule(request: Request, user: dict = Depends(get_current_active_user)):
        return bacnet_app.templates.TemplateResponse("schedule.html", {"request": request, "schedule": bacnet_app.in_memory_schedule, "user": user})

    @app.get("/manage-schedule", response_class=HTMLResponse)
    async def manage_schedule(request: Request, user: dict = Depends(get_current_active_user)):
    ...
    async def update_schedule(request: Request, user: dict = Depends(get_current_active_user)):
        ...
        if error:
            return bacnet_app.templates.TemplateResponse("manage_schedule.html", {"request": request, "error": "End time must be later than start time", "user": user, "schedule": schedule_data})
        
        # Save the updated schedule
        bacnet_app.in_memory_schedule = schedule_data
        # Optionally save to file or database as needed
    ...
    async def check_occupancy():
        ...
        todays_schedule = bacnet_app.in_memory_schedule.get(current_day, {"start": None, "end": None})

        is_occupied = False
        if todays_schedule["start"] and todays_schedule["end"]:
            is_occupied = todays_schedule["start"] <= current_time < todays_schedule["end"]

        return JSONResponse(content={"status": "success", "data": {"current_time": current_time, "current_day": current_day, "is_occupied": is_occupied, "schedule": todays_schedule}})
