...
class AHUBot(EasyASO):
        ...
        if not optimization_status:
            ...
            for ahu in self.ahu_configs:
                if self.is_occupied():
                    ...
                    print(f"Building unoccupied. Skipping pressure control for AHU {ahu.ip}.")
    ...
    async def manage_ahu(self, ahu):
        ...
        if fan_running:
            ...
            print(f"Fan is not running on AHU {ahu.ip}.\nSkipping pressure control this step.")
    ...
    async def read_vav_data(self, ahu):
        ...
        for vav in ahu.vav_configs:
            damper_position = await self.bacnet_read(vav.address, vav.damper_position_obj_id)
            airflow = await self.bacnet_read(vav.address, vav.airflow_obj_id)
            airflow_setpoint = await self.bacnet_read(vav.address, vav.airflow_setpoint_obj_id)
    ...
    async def adjust_static_pressure(self, ahu, vav_data):
        ...
        if total_reset_requests > 0:
            if ahu.total_pressure_increase < ahu.SPres_max:
                pressure_increase = min(ahu.SPres, ahu.SPres_max - ahu.total_pressure_increase)
                ...
                print(f"Maximum pressure increase ({ahu.SPres_max}) reached on AHU {ahu.ip}.")
        else:
...
vav_configs_ahu_1 = [
    VavDefinition(address="10.200.200.233", damper_position_obj_id="analog-input,8", airflow_obj_id="analog-input,1", airflow_setpoint_obj_id="analog-value,4"),
]
...
ahu_configs = [
    AhuDefinition(
        ...
        SP0=1.5, SPmin=0.5, SPmax=3.0, 
        SPtrim=-0.1, SPres=0.2, SPres_max=1.0, 
        I=1, FAN_MIN_SPEED=15.0
    ),
]
