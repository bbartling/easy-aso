...
READ_REQ_INERVAL = 20
FAULT_THRESHOLD = 5
ECONOMIZER_LOW_TEMP = 50
ECONOMIZER_HIGH_TEMP = 68
...
class RCxBot(EasyASO):
    ...
    async def on_step(self):
        ...
        # Check if the current time is not 5 seconds ahead; if not, print current date and time every 60 seconds
        while datetime.now() < current_time + timedelta(seconds=5):
            # If 60 seconds have passed, print the current date and time
            if elapsed_time >= 60:
                print(
                    f"Current time: {datetime.now()}. 60 seconds have passed, waiting for 5 seconds to pass..."
                )
                elapsed_time = 0  # Reset the elapsed time counter after printing

            await asyncio.sleep(2)  # Check every 2 seconds
            elapsed_time += 2  # Increment the elapsed time counter by 2 seconds

        # If 5 seconds have passed, proceed with the rest of the process
        ...
        for _ in range(15):
            ...
            await asyncio.sleep(READ_REQ_INERVAL)
